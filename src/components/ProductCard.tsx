...
export default function ProductCard({ product }: ProductCardProps) {
    ...
    const handleWhatsAppClick = async () => {
        ...
        try {
            ...
            setTimeout(() => setIsLoading(false), 1000); // Reset loading state after 1 second
        }
    };
            ...
            <span className="text-2xl font-bold text-green-600">
              {product.price.toLocaleString('id-ID', {style: 'currency', currency: 'IDR'})}
                ...
                <button
                    onClick={handleWhatsAppClick}
                    disabled={isLoading}
                    className="w-full bg-green-500 hover:bg-green-600 text-white py-3 px-4 rounded-lg font-medium flex items-center justify-center gap-2 transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"
                >
                    {isLoading ? (
                        <>
